...
static const float3 FOG_COLOR 					= float3( 0.5, 0.5, 0.5 ); //地图边框雾气浓度
static const float 	FOG_BEGIN					= 1.0f;
static const float 	FOG_END 					= 150.0f;
static const float 	FOG_MAX 					= 0.35f; //暂不明确（可能是雾气最大值）
...
static const float  SHADOW_WEIGHT_TERRAIN    	= 0.7f; //建筑阴影
...
static const float  SHADOW_WEIGHT_RIVER   		= 0.4f; //河流阴影
...
static const float GB_CAM_MAX = 5.0f;
static const float GB_CAM_MAX_FILLING_CLAMP = 0.95f; // 国土颜色填充透明度（越高越不透明，最高为1），1.0f则为海变黑？
static const float GB_THRESHOLD = 0.6f; // 亮度对比更改（重要）
static const float GB_THRESHOLD2 = 0.5f; // 亮度对比更改（重要）
//static const float3 GB_OUTLINE_COLOR = float3( 0.0f, 0.0f, 0.0f );
static const float GB_OUTLINE_CUTOFF_SEA = 0.990f; // 海洋亮度方面修改（在没有邻居的情况下(海上)平衡边界线）
...
static const float BORDER_FOW_REMOVAL_FACTOR = 0.25f; // 有多少战争迷雾被从边界移除。1.0f 则为没有雾
static const float BORDER_LIGHT_REMOVAL_FACTOR = 1.0f; // 计算从边框中删除了多少的亮度。1.0f则为没有光
static const float GB_STRENGTH_CH1 = 0.8; // 国家颜色不透明度
static const float GB_STRENGTH_CH2 = 0.8; // 顶层不透明度
